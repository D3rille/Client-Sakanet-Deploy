...
const MembersList = ({ getPoolGroupInfoData, poolGroupId }) => {
  ...
  const {
    ...
    error: poolMembersError,
    fetchMore: fetchMoreMembers,
  } = useQuery(GET_POOL_GROUP_MEMBERS, {
    variables: {
      poolGroupId,
      cursor: null,
      limit,
        ...
        updateQuery: (prev, { fetchMoreResult }) => {
          ...
          return Object.assign({}, prev, {
            getPoolGroupMembers: {
              ...
              members: [
                ...prev.getPoolGroupMembers.members,
                ...fetchMoreResult.getPoolGroupMembers.members,
              ],
    ...
    if (isAdmin) {
      subscribeToMoreApplications({
        ...
        updateQuery: (prev, { subscriptionData }) => {
          ...
          return Object.assign({}, prev, {
            getPoolGroupApplications: [
              ...prev.getPoolGroupApplications,
  ...
  if (membershipAppData) {
    var newMemberShipApp = [
      ...new Set(membershipAppData?.getPoolGroupApplications),
    ];
  }
  ...
  return (
    <>
      ...
      >
        <Typography
          variant="body"
          sx={{ fontSize: "0.89em", fontWeight: "bold", textAlign: "left" }}
        >
          Members
        </Typography>
        <Grid
        ...
        >
          ...
          {initialMembersDisplay &&
            initialMembersDisplay?.map((member, index) => (
              <Grid
                item
                xs={4}
                key={member._id}
      ...
      {membershipAppData && poolMembersData && members.length > 0 && (
        <ManageMembersDialog
          isOpen={isModalOpen}
