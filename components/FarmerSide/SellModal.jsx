...
export default function SellModal({ isOpen, onClose }) {
  ...
  const handleDrop = (event) => {
    console.log("Files dropped:", event.target.files);
    setFile(event.target.files[0]);
  };

  const handleClose = () => {
    if(productName || price || stocks || description || selectedDate || file) {
  ...
  const allFieldsFilled = productName && price && stocks && description && selectedDate && file;
