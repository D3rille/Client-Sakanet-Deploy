...
const EditReviewModal = ({ isOpen, onClose, handleEditReview, myReviewData}) => {
  const [overallRating, setOverallRating] = useState(myReviewData.rate ?? 0);
  const [writtenReview, setWrittenReview] = useState(myReviewData.comment ?? "");
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          ...
          <Button
            onClick = {()=>{
                handleEditReview(myReviewData._id, overallRating, writtenReview).then(()=>{
                    onClose()
                });
